...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderFeed() {
    ...
    filtered.forEach((release, index) => {
      ...
      console.log("Rendering release:", release.title);
  ...
  function selectRelease(release) {
    ...
    const detailContainer = document.querySelector(".detail-body-container");
    if (detailContainer) {
      detailContainer.scrollTop = 0;
    }

    // Force external links inside release note to open in a new tab
    docBody.querySelectorAll("a").forEach((link) => {
  ...
  function handleKeyboardNav(e) {
    ...
    // Ignore keydown if no releases are loaded
    if (!state.filteredReleases || state.filteredReleases.length === 0) return;
    ...
    if (e.key === "ArrowDown" || e.key === "ArrowUp") {
      ...
      if (state.selectedRelease) {
        const currentIndex = state.filteredReleases.findIndex(
          (r) => r.id === state.selectedRelease.id
        );
      ...
      if (nextIndex >= 0 && nextIndex < state.filteredReleases.length) {
        const releaseToSelect = state.filteredReleases[nextIndex];
        selectRelease(releaseToSelect);
        
        // Scroll active item into view inside the feed log list
        setTimeout(() => {
          const activeEl = feedList.querySelector(".feed-item.active");
          if (activeEl) {
